...
import { Vector3, MathUtils as THREEMath, Euler } from 'three';
...
export const canvasGenerator = AFRAME.registerComponent('canvas-generator', {
    init: function(): void {
        ...
        canvasEl.classList.add('movable');
        ...
        this.el.addEventListener('showcanvas', (event) => {
            ...
            if (this.el.object3D.visible) {
                ...
                const boundingBox = new THREE.Box3().setFromObject(canvas.object3D);
                const width = boundingBox.max.x - boundingBox.min.x;
                console.log(width);
                const position = this.mainCam.object3D.localToWorld(new Vector3(0, 1.6, -1.2));
                ...
                this.el.object3D.position.set(position.x, camPosition.y + 0.8, position.z);
